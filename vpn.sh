...
export PKI_DAT="$PKI_DIR/pki.dat"
...
export VPN_TLS_CIPHER=""


##############################
# Create the environment file
##############################
#echo "IPV4_ADDR=$IPV4_ADDR" > $ENV_CNF
#echo "PKI_SERVER=$PKI_SERVER" >> $ENV_CNF
#echo "PKI_SIGNER=$PKI_SIGNER" >> $ENV_CNF
#echo "VPN_PID=$VPN_PID" >> $ENV_CNF
...
cat $PKI_DIR/crt/$REQ_CN $PKI_DIR/key/$REQ_CN > $PKI_DIR/$REQ_CN
...
# Diffie-Hellman parameters while we're at it
...
[ -n "$VPN_TLS_CIPHER" ] && echo "tls-cipher $VPN_TLS_CIPHER" >> "$VPN_CNF"
[ -n "$VPN_CIPHER" ] && echo "cipher $VPN_CIPHER" >> "$VPN_CNF"
[ -n "$VPN_AUTH_ALGO" ] && echo "auth $VPN_AUTH_ALGO" >> "$VPN_CNF"
[ -n "$VPN_CLIENT_TO_CLIENT" ] && echo "client-to-client" >> "$VPN_CNF"
[ -n "$VPN_COMP_LZO" ] && echo "comp-lzo" >> "$VPN_CNF"
...
<key>
EOF
openssl pkey -in $PKI_DIR/key/$PKI_CLIENT -outform pem >> $PKI_DAT
echo "</key>" >> $CLIENT_CNF

echo "<cert>" >> $CLIENT_CNF
openssl x509 -in $PKI_DIR/crt/$PKI_CLIENT -outform pem >> $PKI_DAT
echo "</cert>" >> $CLIENT_CNF

echo "<ca>" >> $CLIENT_CNF
openssl x509 -in $PKI_DIR/crt/$PKI_SIGNER -outform pem >> $PKI_DAT
echo "</ca>" >> $CLIENT_CNF

echo "<tls-auth>" >> $CLIENT_CNF
openssl pkey -in $VPN_TLS_AUTH -outform pem >> $PKI_DAT
echo "</tls-auth>" >> $CLIENT_CNF
